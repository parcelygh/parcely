...
- `@postalservice/auth-token` addon for token attachment and 401 refresh with single-flight coalescing.
- `@postalservice/auth-redirect` addon for browser login redirects on 401/403.
