...
When `tls` is set in `RequestConfig`, postalservice lazy-imports `undici` (built into Node 20+) and creates a custom `Agent` with the specified TLS options. This agent is passed to `fetch` as a dispatcher.
