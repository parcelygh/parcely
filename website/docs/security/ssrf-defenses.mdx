...
When a `baseURL` is configured, postalservice rejects any `url` that is absolute (starts with a protocol like `http:` or `https:`, or is protocol-relative like `//host/path`). This is controlled by:
...
Even without a `baseURL`, postalservice restricts which URI schemes are permitted:
