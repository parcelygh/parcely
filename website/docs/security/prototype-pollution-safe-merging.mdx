...
postalservice's config merging (`mergeConfig`) uses an explicit allowlist copy. Only known `RequestConfig` keys are copied from the override object. Additionally, when merging nested objects (like `headers` or `params`), the following keys are stripped:
