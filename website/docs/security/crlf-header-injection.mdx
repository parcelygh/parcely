...
postalservice constructs all request headers using the native `Headers` API. The `Headers` constructor and `set`/`append` methods validate header names and values, rejecting any that contain `\r`, `\n`, or `\0` characters.

If a CRLF injection is detected, postalservice throws `HttpError` with `code: 'ERR_CRLF_INJECTION'`.
