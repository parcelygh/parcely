...
postalservice is designed with 13 security defaults that are enabled out of the box. These defaults address every class of vulnerability found in axios CVEs over the past several years.
...
| 4 | Manual redirect walk with `maxRedirects` | `maxRedirects: 5` | Redirects are followed manually (not by the browser/runtime), giving postalservice control over header stripping and redirect limits. |
...
- HSTS enforcement
- Automatic retries (planned: `@postalservice/retry`)
