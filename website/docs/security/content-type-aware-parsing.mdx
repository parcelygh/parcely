...
postalservice checks the response `Content-Type` header before attempting JSON parsing. When `responseType` is `'json'` (the default):
...
3. If JSON parsing fails even with a JSON content type, postalservice throws `HttpError` with `code: 'ERR_PARSE'` and the original `SyntaxError` as `cause`.
