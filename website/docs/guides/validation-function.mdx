...
postalservice's `Validator<T>` type accepts `(input: unknown) => T` functions. The function receives the parsed response body. If it returns a value, that value becomes `data`. If it throws, postalservice wraps the error in `HttpError` with `code: 'ERR_VALIDATION'`.
