...
You want both a per-request timeout and the ability to cancel manually. postalservice handles this automatically.
...
Internally, postalservice uses `AbortSignal.any([userSignal, AbortSignal.timeout(ms)])` to combine both signals. Whichever fires first wins:
...
- If only `timeout` is set (no `signal`), postalservice uses `AbortSignal.timeout(ms)` directly.
