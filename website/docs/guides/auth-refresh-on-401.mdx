...
## Using `@postalservice/auth-token` (recommended)
...
The axios pattern is essentially the same interceptor approach. `@postalservice/auth-token` removes the boilerplate.

## Notes and gotchas

- Use a **separate client** for the refresh call to avoid interceptor loops.
- The `@postalservice/auth-token` addon handles single-flight coalescing and retry bounding automatically.
- If you install both `@postalservice/auth-token` and `@postalservice/auth-redirect`, install auth-token first. The redirect runs after refresh has failed.
