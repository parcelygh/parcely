...
Use postalservice in a React application, including custom hooks and integration with TanStack Query or SWR.
...
- A dedicated `@postalservice/react` package with `useHttp()` hooks is planned for the future.
- postalservice's `signal` option integrates naturally with TanStack Query's `signal` parameter.
