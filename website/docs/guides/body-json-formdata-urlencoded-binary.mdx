...
When a plain object contains any `File` or `Blob` value, postalservice automatically converts it to `FormData`:
...
In axios, the body parameter is called `data` in the config. In postalservice, it is `body` in the config, but method helpers (`.post()`, `.put()`, `.patch()`) accept it as the second positional argument just like axios:

```ts
// Both work the same way positionally:
// axios:          http.post('/url', bodyData, config)
// postalservice:  http.post('/url', bodyData, config)
```
...
- If you manually set `Content-Type`, postalservice respects it and skips auto-detection.
