...
postalservice creates an `AbortSignal.timeout(ms)` and combines it with any user-provided `signal` via `AbortSignal.any`. When the timeout fires, the request throws `HttpError` with `code: 'ERR_TIMEOUT'`.
...
In axios, the timeout error has `code: 'ECONNABORTED'`. In postalservice it is `code: 'ERR_TIMEOUT'`.
