...
When `tls` is set, parcely lazy-imports `undici` (built into Node 20+) and creates a custom `Agent` with the specified TLS options. This is passed to `fetch` as a dispatcher.
