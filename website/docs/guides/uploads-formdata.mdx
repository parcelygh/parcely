...
When a plain object contains `File` or `Blob` values, postalservice automatically converts it to `FormData`:
...
Axios auto-converts plain objects with File/Blob to FormData via `formSerializer`. postalservice uses `formDataSerializer` with the same default (`'brackets'`).
...
- If you manually set `Content-Type`, postalservice respects it and skips auto-conversion.
