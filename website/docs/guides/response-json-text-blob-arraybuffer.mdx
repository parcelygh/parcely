...
Both libraries support `'json'`, `'text'`, `'arraybuffer'`, and `'blob'`. parcely also supports `'stream'` (returns the un-consumed `ReadableStream` — see the [download streaming guide](./download-streaming.mdx)). Axios supports `'stream'` (Node `Readable`) and `'document'` (browser DOM); parcely does not provide `'document'`.
