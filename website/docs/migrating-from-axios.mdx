...
postalservice is designed so axios users feel at home immediately. This guide shows every common axios pattern alongside its postalservice equivalent, and includes a full feature-mapping table at the bottom.
...
<TabItem value="postalservice" label="postalservice">
...
**Key differences:**
- postalservice uses `isHttpError()` instead of `axios.isAxiosError()`.
- Error codes are different: postalservice uses descriptive codes like `ERR_HTTP_STATUS`, `ERR_TIMEOUT`, `ERR_NETWORK`, `ERR_ABORTED`, `ERR_VALIDATION`, `ERR_PARSE`, `ERR_TOO_MANY_REDIRECTS`, `ERR_DISALLOWED_PROTOCOL`, `ERR_DISALLOWED_HEADER`, `ERR_ABSOLUTE_URL`, `ERR_CRLF_INJECTION`.
...
**Key difference:** postalservice configs are plain objects -- spread and return a new object rather than mutating. Error handlers can `throw` instead of `return Promise.reject()`.
...
Identical. postalservice does not support the legacy `CancelToken` API.
...
In postalservice, timeout is combined with a user-provided `signal` via `AbortSignal.any`, so both can coexist cleanly.
...
postalservice progress events include a `percent` field (0--100) when the total is known.
...
postalservice supports `'json'` (default), `'text'`, `'arraybuffer'`, and `'blob'`. The axios `'stream'` and `'document'` types are not provided.

## Runtime validation (new in postalservice)

postalservice adds a `validate` option with no axios equivalent:
...
| axios config key | postalservice equivalent | Notes |
|---|---|---|
...
| `adapter` | Not provided | postalservice always uses `fetch`. For testing, stub `globalThis.fetch`. |
| `auth` | Not provided as config | Use `headers: { Authorization: 'Bearer ...' }` or `@postalservice/auth-token`. |
...
| `withCredentials` | Not provided | Set `credentials: 'include'` via a request interceptor if needed. postalservice does not wrap the fetch `credentials` option. |
